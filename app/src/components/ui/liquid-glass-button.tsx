...
function LiquidButton({
  ...
  const Comp = asChild ? Slot : "button"
  // Each instance gets its own filter id (React.useId) — the original
  // upstream snippet hardcodes id="container-glass", which collides (SVG ids
  // are global) the moment two LiquidButtons render on the same page, so
  // every button after the first silently loses its distortion filter.
  const filterId = `liquid-glass-filter-${React.useId()}`

  // Structural fix vs. the original upstream snippet: that version nested
  // the decorative shadow/backdrop divs AND {children} all as children of
  // <Comp>. That's fine for the plain "button" case, but when asChild=true,
  // Comp becomes Radix's <Slot>, which requires EXACTLY ONE child element —
  // passing 3-4 children throws "React.Children.only" at render time, so
  // `<LiquidButton asChild><Link .../></LiquidButton>` (the whole point of
  // asChild — making the CTA a real, crawlable <a>, not a button with a
  // manual router.push) would crash. Fix: the decorative layers move to an
  // outer plain <span>, sibling to <Comp> instead of nested inside it, so
  // Slot's single-child contract holds and asChild genuinely works.
  return (
